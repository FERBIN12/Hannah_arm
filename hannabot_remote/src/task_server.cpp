...
#include "hannahbot_msgs/action/arduino_bot_task.hpp"
...
public:
    ...
    : Node("task_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<hannahbot_msgs::action::ArduinoBotTask>(
...
private:
  rclcpp_action::Server<hannahbot_msgs::action::ArduinoBotTask>::SharedPtr action_server_;
  ...
  rclcpp_action::GoalResponse goalCallback(
      ...
      std::shared_ptr<const hannahbot_msgs::action::ArduinoBotTask::Goal> goal)
  {
    RCLCPP_INFO(get_logger(), "Received goal request with id %d", goal->tasknumber);
    (void)uuid;
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
  }

  rclcpp_action::CancelResponse cancelCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<hannahbot_msgs::action::ArduinoBotTask>> goal_handle)
  {
    (void)goal_handle;
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<hannahbot_msgs::action::ArduinoBotTask>> goal_handle)
  {
    RCLCPP_INFO(get_logger(), "Executing goal");
    auto result = std::make_shared<hannahbot_msgs::action::ArduinoBotTask::Result>();
    ...
    if (goal_handle->get_goal()->tasknumber == 0)
    {
      arm_joint_goal = {0.0, 0.0, 0.0};
      gripper_joint_goal = {-0.7, 0.7};
    }
    else if (goal_handle->get_goal()->tasknumber == 1)
    {
      arm_joint_goal = {-1.14, -0.6, -0.07};
      gripper_joint_goal = {0.0, 0.0};
    }
    else if (goal_handle->get_goal()->tasknumber == 2)
    {
...
RCLCPP_COMPONENTS_REGISTER_NODE(hannabot_remote::TaskServer)
